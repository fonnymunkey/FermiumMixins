package fermiummixins.mixin.neat;
...
public abstract class HealthBarRenderer_OptifineMixin {
    ...
    @Shadow(remap = false)
    public abstract void renderHealthBar(EntityLivingBase passedEntity, float partialTicks, Entity viewPoint);

    @Unique
    private static Field fermiummixins$entityListField;
    ...
    public void onRenderWorldLast(RenderWorldLastEvent event) {
        ...
        if ((NeatConfig.renderInF1 || Minecraft.isGuiEnabled()) && NeatConfig.draw) {
            ...
            if (NeatConfig.showOnlyFocused) {
                ...
                try {
                    if(fermiummixins$entityListField == null) {
                        fermiummixins$entityListField = ObfuscationReflectionHelper.findField(WorldClient.class, "field_73032_d");
                        fermiummixins$entityListField.setAccessible(true);
                    }
                    var15 = ((Set<Entity>)fermiummixins$entityListField.get(client)).iterator();
                }
