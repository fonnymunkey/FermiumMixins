...
public abstract class NameGeneratorMixin extends WorldSavedData {
	...
	@Unique
	private final Object2IntMap<String> fermiummixins$usedNamesMap = new Object2IntOpenHashMap<String>();
	...
	public String getName(BlockPos pos, int dimension, Biome biome, Random rand) {
		...
		for(String tryName : customNames) {
			if(!this.fermiummixins$usedNamesMap.containsKey(tryName)) {
		...
		if(name == null) {
			...
			if(ConfigHandler.WAYSTONES_CONFIG.villageWaystoneRemoveBiome) {
			...
			count = this.fermiummixins$usedNamesMap.getInt(name);
		...
		this.fermiummixins$usedNamesMap.put(baseName, count+1);
		this.markDirty();
		return name;
	}
	...
	public void readFromNBT(NBTTagCompound compound) {
		...
		for(int i = 0; i < tagList.tagCount(); ++i) {
			...
			this.fermiummixins$usedNamesMap.put(comp.getString("Name"), comp.getInteger("Val"));
		}
	}
	...
	public NBTTagCompound writeToNBT(NBTTagCompound compound) {
		...
		for(Map.Entry<String, Integer> entry : this.fermiummixins$usedNamesMap.entrySet()) {
