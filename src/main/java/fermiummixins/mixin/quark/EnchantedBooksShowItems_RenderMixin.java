...
public abstract class EnchantedBooksShowItems_RenderMixin {
    ...
    public void makeTooltip(ItemTooltipEvent event) {
        ...
        if(stack.getItem() == Items.ENCHANTED_BOOK || stack.getItem() == AncientTomes.ancient_tome) {
            ...
            int wrapCount = ConfigHandler.QUARK_CONFIG.enchantedTooltipWrapCount;
            ...
            for(EnchantmentData ed : enchants) {
                ...
                for(; tooltipIndex < tooltip.size(); tooltipIndex++)
                    if(tooltip.get(tooltipIndex).equals(match)) {
                        ...
                        if(!items.isEmpty()) {
                            int index = 0;
                            for(; index < items.size()/ wrapCount; index++) {
                                int len = 3 + wrapCount * 9;
                                String spaces = "";
                            ...
                            if(items.size()% wrapCount > 0) {
                                int len = 3 + items.size()% wrapCount * 9;
                                String spaces = "";
                                while(mc.fontRenderer.getStringWidth(spaces) < len)
                                    spaces += " ";
                ...
                for(int tooltipIndex = 0; tooltipIndex < tooltip.size(); tooltipIndex++) {
                    ...
                    if(line != null && line.equals(match)) {
                        ...
                        for(ItemStack testStack : items) {
                            mc.getRenderItem().renderItemIntoGUI(testStack, 6 + (drawn% wrapCount) * 18, (tooltipIndex+(drawn/ wrapCount)) * 20 - 2);
                            drawn++;
                        }
