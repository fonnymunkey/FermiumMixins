		...
		public boolean isItemValid(ItemStack stack) {
			if(stack != null && !stack.isEmpty()) {
				if(stack.getItem().isValidArmor(stack, equipmentSlot, player)) {
					return Arrays.asList(ConfigHandler.COSMETICARMORREWORKED_CONFIG.cosmeticArmorItemBlacklist).contains(stack.getItem().getRegistryName().toString()) == ConfigHandler.COSMETICARMORREWORKED_CONFIG.cosmeticArmorItemBlacklistIsWhitelist;
				}
			}
			return false;
		}
