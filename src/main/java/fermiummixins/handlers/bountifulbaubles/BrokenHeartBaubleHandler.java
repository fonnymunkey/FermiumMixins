...
public class BrokenHeartBaubleHandler {
    ...
    public static void onFirstAidLivingDamageLow(FirstAidLivingDamageEvent event) {
        ...
        if(BaublesApi.isBaubleEquipped(event.getEntityPlayer(), ModItems.trinketBrokenHeart) == -1) return;
        ...
        if(!failed && !parts.isEmpty()) {
            ...
            AttributeModifier modifier = maxHealth.getModifier(BROKENHEART_HEALTH_UUID);
