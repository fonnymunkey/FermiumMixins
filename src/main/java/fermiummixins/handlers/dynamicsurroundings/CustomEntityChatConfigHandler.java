...
public abstract class CustomEntityChatConfigHandler {
    ...
    private static final Map<String, String> customEntityChatMap = new HashMap<>();
    private static final Map<String, int[]> customEntityChatTimeMap = new HashMap<>();

    @Nullable
    public static Map<String, String> getCustomEntityChatMap(String lang) {
        if(customEntityChatFile == null && !initCustomEntityChatMap(lang)) return null;
        return customEntityChatMap;
    }

    @Nullable
    public static Map<String, int[]> getCustomEntityChatTimeMap() {
        if(customEntityChatTimeFile == null && !initCustomEntityChatTimeMap()) return null;
    ...
    private static boolean initCustomEntityChatMap(String lang) {
        ...
        if(dSurroundLangChatFile.exists()) customEntityChatFile = dSurroundLangChatFile;
        else customEntityChatFile = new File(configFolder, "dsurround_chat.cfg");
        try {
            ...
            if(!customEntityChatTimeFile.exists()) {
                if(!customEntityChatTimeFile.createNewFile()) {
                    ...
                    return false;
                        ...
                        customEntityChatTimeFile.toPath(),
                        (       "# Format:\n" +
                                "#\n" +
    ...
    private static boolean initDirectory() {
        configFolder = new File("config", FermiumMixins.MODID);
